...
#index = 14
#blocks = np.array([blocks[index]])
#pages_in_block = np.array([pages_in_block[index]])
numBlocks = len(blocks)
#print(blocks)

mainURL = 'http://gravar.se/en/forsamling/mala-sorsele-pastorat/' + cemetery_name + '/0/'
#block_links = np.array([])
#block_names = np.array([])
#block_births = np.array([])
#block_deaths = np.array([])
#block_sites = np.array([])
#block_datesnums = np.array([])
#%%
entries = []
...
for b in range(numBlocks):
    ...
    for i in range(pages_in_block[b]):
        pnum = str(i+1)
